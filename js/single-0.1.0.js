(function ($) {
    ...
    function changeCSS(element) {
        ...
        $(element).css({
            "width": windowWidth,
            "height": windowHeight + targetsSize + targetsSize
        });
...
//THERE YO CAN CHANGE THE DISTANCE TO TOP OF DIV WHERE YOU WANT TO MOVE
        ...
        if ($.browser.msie) {
            targetPosition = $('[data-target="' + goingTo + '"]').position().top - 50; // get the position of the target
        } else {
            targetPosition = $('[data-target="' + goingTo + '"]').position().top - 80; // get the position of the target
        }
